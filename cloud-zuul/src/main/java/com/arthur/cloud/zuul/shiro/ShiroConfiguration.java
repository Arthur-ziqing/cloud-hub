package com.arthur.cloud.zuul.shiro;

import com.arthur.cloud.common.config.redis.RedisConfig;
import com.arthur.cloud.zuul.filters.AuthFilter;
...
public class ShiroConfiguration {
    ...
    public ShiroFilterFactoryBean shiroFilterFactoryBean(SecurityManager securityManager) {
        ...
        // 配置需要验证登录后访问的链接
        shiroFilterFactoryBean.setLoginUrl(webpageurl + "/web/web-user/to-login");

        //自定义过滤器
        Map<String, Filter> filters = new LinkedHashMap<>();
        filters.put("authc", new AuthFilter());
        LogoutFilter logoutFilter = new LogoutFilter();
        logoutFilter.setRedirectUrl(webpageurl + "/web/web-user/to-login");
        filters.put("logout", logoutFilter);
        Map<String, String> filterChainDefinitionMap = new LinkedHashMap<>();
        // 配置不会被拦截的链接 顺序判断
        filterChainDefinitionMap.put("/web/web-user/login", "anon");
    ...
    public RedisManager redisManager() {
        ...
        redisManager.setExpire(604800);// 配置缓存过期时间
